...
export const PastWeekReviewsChart = ({ reviews }: { reviews: Review[] }) => {
  ...
  const lastWeekDays = () => {
    return Array.from({ length: 7 }, (_, i) => dayjs().subtract(6 - i, "day").format("ddd"));
  }

  const reviewsPerDay = lastWeekDays().map(day => {
    if (!reviews || reviews.length == 0) return 0;
    return reviews.filter(review => dayjs(review.card_object.last_review).format("ddd") === day).length;
  });
  ...
  return (
    <Box sx={{ my: 2 }}>
      ...
      <BarChart
        xAxis={[{ id: "xAxis", data: lastWeekDays(), label: "Day of the Week" }]}
        yAxis={[{ id: "yAxis", tickMinStep: 1, data: [1, 2, 3, 4, 5, 6, 7], label: "Lessons Reviewed" }]}
        series={[
          {
            data: reviewsPerDay,
